...
const MobileHomePage = () => {
  ...
  const [selectedStyle, setSelectedStyle] = useState({ id: 'default', name: '默认风格', desc: '原生艺术呈现', icon: '✨', pts: 'All', hint: '描述你想生成的画面', placeholder: '主题：【在此输入你想生成的画面】' });
  ...
  const handleGenerate = async (forceNew = false) => {
    ...
    setJobs(prev => [...prev, newJob]); // 追加到末尾，配合滚动到底部逻辑
    setPrompt('');
    try {
      ...
      const errorDetail = err.response?.data?.detail || '提交失败，请检查积分或网络';
      setJobs(prev => prev.map(j => j.id === newJob.id ? { ...j, status: 'failed', error: errorDetail } : j));
    }
  };
  ...
  return (
    <div style={{ 
      ...
      <div style={{ padding: '16px 16px 24px', background: 'linear-gradient(to top, rgba(255,255,255,0.9) 60%, transparent)', position: 'relative' }}>
        ...
        {/* 风格输入引导 */}
        {!enhancing && (
          <div style={{ padding: '0 20px', marginBottom: '8px', fontSize: '12px', color: selectedStyle.requiresImage && !refImageUrl ? '#e66b33' : '#999', fontWeight: '500' }}>
