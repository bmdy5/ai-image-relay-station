...
STYLE_PROMPT_TEMPLATES = {
    ...
    "virtual_tryon": "电商模特宣传照。严格保留参考图中人物的五官、脸型、肤色、发型、姿势和身体比例不变。给人物穿上【主题】。关键要求：服饰必须贴身合体，自然勾勒身形曲线，不得松垮悬空。领口、袖长、腰线、裙摆的版型与描述严格一致，不能自行改变款式。面料质感真实垂坠，随身体动作产生合理自然褶皱，无穿模感。专业摄影棚柔和均匀布光，干净浅色背景（白色或浅灰）。构图：半身或七分身，正面略侧15度，眼神自然看向镜头，表情自信放松。整体呈现专业电商服装模特图的质感。"
}
...
def wrap_prompt(style_id: str, raw_prompt: str, quality: str) -> str:
    ...
    if style_id in ["vintage_5s", "ccd_snap", "real", "restore_old"]:
        realism_patch = (
            " (人像核心要求：构图需自然多样（支持全身照、坐姿、站姿、侧身等多种生动角度），服饰搭配需高度统一、精致且具有质感。"
            "画面呈现必须完全自然，禁止过度锐化或假脸美颜，保留皮肤毛孔细节，确保光学镜头般的写实度与肢体协调感。) "
        )
